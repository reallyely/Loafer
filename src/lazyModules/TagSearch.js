...
const maybeFilter = (value, addedTags) =>
...
export class TagSearch {
  constructor(initialProps) {
    ...
    this.actions = {
      addTag: () => {
        return store => {
          console.log("hit tagserach!", store);
          return {
            value: "",
            possibleTags: [],
            selectedPossibleTag: 0,
      ...
      clearPossibleTags: () => store => ({ possibleTags: [] }),
      calcPossibleTags: value => store => ({
        value,
        possibleTags: maybeFilter(value, store.addedTags),
        selectedPossibleTag: 0
      }),
      ...
      backspaceTag: value => store => ({
        possibleTags: maybeFilter(value, store.addedTags),
        addedTags:
          store.value.length === 0 && store.addedTags.length > 0
      ...
      selectNextSuggestedTag: () => store => ({
        selectedPossibleTag:
          store.selectedPossibleTag < store.possibleTags.length - 1
      ...
      selectPreviousSuggestedTag: () => store => ({
        selectedPossibleTag:
          store.selectedPossibleTag > 0 ? store.selectedPossibleTag - 1 : 0
      })
    };
  }
}
